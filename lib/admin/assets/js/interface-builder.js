	...
	var initMediaLibraryElement = function(){
			...
			cherry_uploader.on('select', function() {
				...
				while(attachment[count]){
					...
					if( img_data.sizes != undefined){
						...
						img_src = img_data.icon;
					}

					new_img += '<div class="cherry-image-wrap"><img  src="' +img_src+ '" alt="" data-img-attr="'+return_data+'"><a class="cherry-remove-image" href="#"><i class="dashicons dashicons-no"></i></a></div>';
		...
		var removeMediaPreview = function( item ){
			var button_parent = item.parents('.cherry-section'),
				input = jQuery('.cherry-upload-input', button_parent),
				img_holder = item.parent('.cherry-image-wrap'),
			...
			update: function(event, ui) {
				var attachment_ids = '';
					jQuery('.cherry-image-wrap', this).each(
						function() {
							var attachment_id = jQuery('img', this).data( 'img-attr' );
